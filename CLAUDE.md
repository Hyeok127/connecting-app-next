...
- **사진**: 가입 시 받지 않는다. 매칭 **성사 후** 양측이 모두 동의해야 교환된다
  ...
  - **남은 것은 클라우드 DB에 004 DDL 적용 하나** — service_role 키로는 DDL 불가라
    자동화 불가(대시보드 SQL Editor 또는 psql 필요). 컬럼 없으면 photo-consent POST만
    실패하고 나머지는 미노출로 안전하므로, 릴리스 체크리스트대로 dev→운영 순서로 적용한다.
