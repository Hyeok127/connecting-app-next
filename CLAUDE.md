...
- **사진**: 가입 시 받지 않는다. 매칭 **성사 후** 양측이 모두 동의해야 교환된다
  (`matches.a_photo_consent`/`b_photo_consent`, 마이그레이션 004). 동의 API는
  `POST /api/matches/[id]/photo-consent` — 본인 사진(프로필 등록)이 있어야 동의 가능.
  매칭 목록 API는 `photos_exchanged`가 참일 때만 상대 photos를 포함한다(이전엔 pending
  단계부터 사진을 노출했음 — 구 §3-4 규칙 폐기).
  - **검증(2026-08-11)** — 긍정·부정 경로를 실환경 두 축으로 확인:
    1. 부정 경로 (실 클라우드 dev DB, STAGE 1 E2E): 사진 없는 가입 → 근무지/연락처 미수집
       → 배치 매칭 → 양측 수락 → 성사 후에도 동의 전 사진 미노출·연락처만 공개. 컬럼이
       없는 현 클라우드 상태에서도 "사진 미노출"이 안전 기본값임을 실증(fail-safe).
    2. 긍정 경로 게이트 (pg-mem, 004 파일 실제 적용): 진리표
       `(0,0)/(1,0)/(0,1)→미노출, (1,1)→노출` 통과 — 마이그레이션 SQL 유효성 + 게이트 로직.
    3. 긍정 경로 전달 (실 클라우드 dev Storage): 업로드→서명 URL→실제 사진 바이트 전달
       (HTTP 200), 서명 없는 접근 차단(400). "전달" 메커니즘이 클라우드에서 실동작.
    4. **통합 (실코드+실클라우드)**: 실제 `lib/serialize.ts`(esbuild로 타입만 제거해 번들,
       로직 그대로)를 라우트의 실제 게이트 판정으로 구동 → (1,0)은 photos 미포함,
       (1,1)은 photos 포함 + 그 서명 URL이 실 클라우드 dev Storage에서 HTTP 200으로
       실제 도달(양측 시점 모두). "쌍방 동의→사진 전달" 긍정 경로가 실코드+실클라우드로
       통합 검증됨.
  - **배포 준비 관점에서 코드/기능 검증은 완료.** 남은 건 순수 배포 액션 하나 —
    클라우드 DB(dev/운영)에 004의 컬럼 2개 추가(additive). service_role 키로는 DDL이
    불가라 대시보드 SQL Editor/psql로 실행해야 하며, 이는 운영 마이그레이션과 동일하게
    릴리스 런북 항목이다. 컬럼 없으면 photo-consent POST만 실패하고 나머지는 미노출로
    안전(fail-safe, STAGE 1로 확인). 적용 순서는 릴리스 체크리스트 참조.
...
## 릴리스 체크리스트 — 개인정보 최소화판 (2026-08-11 준비)

dev 검증 후 운영 반영 시 이 순서대로. **1→2 순서 중요** (코드가 먼저 나가면
photo-consent API가 운영에서 컬럼 없음 에러).

1. **운영 DB에 004 적용** — Supabase 대시보드에서 **Connecting(`pcoxykee...`, Seoul)**
   선택(⚠️ dev와 혼동 주의) → SQL Editor → `004_photo_exchange.sql` 실행.
   additive 컬럼이라 기존 데이터·구버전 코드에 무해 — 배포 전에 먼저 실행해둔다.
2. **`dev` → `main` 머지 + push** → Vercel Production 자동 배포.
3. **배포 직후 확인**: 프로덕션 URL 200, 가입 폼에 사진 필드 없음, 기존 계정 로그인 정상.
4. **기존 회원 공지(수동)**: 기존 회원은 본명으로 가입돼 있다 — 닉네임으로 바꾸려면
