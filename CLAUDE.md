...
- **사진**: 가입 시 받지 않는다. 매칭 **성사 후** 양측이 모두 동의해야 교환된다
  ...
  - **검증(2026-08-11)**: 부정 경로는 실 dev DB E2E(STAGE 1)로 통과 — 성사 후에도 동의
    전엔 사진 미노출·연락처만 공개. 긍정 경로는 `pg-mem`에 004를 실제 적용해 진리표
    `(0,0)/(1,0)/(0,1)→미노출, (1,1)→노출`을 검증(STAGE 2). **004는 아직 클라우드 dev/운영
    DB에는 미적용** — 릴리스 체크리스트 1번대로 대시보드/psql로 적용해야 photo-consent
    POST가 클라우드에서 동작한다(컬럼 없으면 그 API만 실패, 나머지는 사진 미노출로 안전).
